...
def mock_anomaly_dependencies(monkeypatch):
    ...
    dummy_models = LoadedModels(
        anomaly_model="dummy", anomaly_version="1.0",
        rul_model="dummy", rul_version="1.0"
    )
    ...
    monkeypatch.setattr("anomaly_service.main.preprocess_anomaly", lambda s, vals, sch, art: ("x", 0))
...
def test_predict_anomaly_missing_feature_graceful_fallback(client, sample_anom_request, monkeypatch):
