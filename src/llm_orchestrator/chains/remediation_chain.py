...
def _extract_missing_entities(user_query: str | None, current_eq: str | None, current_fail: str | None) -> tuple[str | None, str]:
    eq_id = current_eq
    ...
    if user_query:
        ...
        if not failure:
            q_lower = user_query.lower()
            if "bearing" in q_lower: failure = "bearing_failure"
            elif "cavitation" in q_lower: failure = "cavitation"
            elif "recalibrate" in q_lower or "sensor" in q_lower: failure = "sensor_calibration"
            else: failure = "general_maintenance"
...
def _sanitize_list(items: list[str], doc_mapping: dict[str, str]) -> list[str]:
    """
    PRODUCTION FIX: Iterates through arrays and applies Context Sanitization,
    replacing raw DOC_X tags with human-readable file names.
    """
    sanitized = []
    for item in items:
        new_item = item
...
class RemediationChain:
    ...
    async def run(self, req: RemediationRequest) -> tuple[RemediationResponse, str, str, str]:
        req.equipment_id, req.failure_mode = _extract_missing_entities(req.user_query, req.equipment_id, req.failure_mode)
        ...
        if not docs:
            fallback_response = RemediationResponse(
                safety_warnings=[f"No technical documentation was retrieved for {req.failure_mode}, so safe repair guidance cannot be provided from the available context."],
                tools_required=[],
                steps=[],
                sources=[]
            )
            return fallback_response, "openai", "gpt-5.4-mini", "No context retrieved."
        ...
        # PRODUCTION FIX: Apply sanitization to all textual arrays
